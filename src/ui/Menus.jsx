...
function Menus({ children }) {
  ...
  return (
    <MenusContext.Provider
      value={{ openId, close, open, setMenuPosition, menuPosition }}
    >
      {children}
    </MenusContext.Provider>
  );
...
function Toggle({ id }) {
  const { open, close, openId, setMenuPosition } = useContext(MenusContext);
...
function List({ id, children }) {
  ...
  const ref = useOutsideModalClose(close);
